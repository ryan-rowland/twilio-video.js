...
class PeerConnectionV2 extends StateMachine {
  ...
  constructor(id, encodingParameters, preferredCodecs, options) {
    ...
    Object.defineProperties(this, {
      ...
      _shouldUpdateBitrateParameters: {
        writable: true,
        value: typeof encodingParameters.maxAudioBitrate === 'number'
          || typeof encodingParameters.maxVideoBitrate === 'number'
      },
    ...
    encodingParameters.on('changed', oncePerTick(() => {
      if (isRTCRtpSenderParamsSupported) {
        if (this._needsAnswer) {
          this._shouldUpdateBitrateParameters = true;
        } else {
...
function negotiationCompleted(pcv2) {
  ...
  if (isRTCRtpSenderParamsSupported && pcv2._shouldUpdateBitrateParameters) {
    updateEncodingParameters(pcv2);
    pcv2._shouldUpdateBitrateParameters = false;
  }
}
